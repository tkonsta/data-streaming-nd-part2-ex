...
customerScoreSchema = StructType (
    [
...
spark = SparkSession.builder.appName("stedi-application").getOrCreate()
...
zSetEntriesEncodedStreamingDF=spark.sql("select key, zSetEntries[0].element as redisEvent from RedisSortedSet where key is not null and zSetEntries[0].element is not null and key = 'Q3VzdG9tZXI='")
...
zSetEntriesDecodedStreamingDF= zSetEntriesEncodedStreamingDF.withColumn("redisEvent", unbase64(zSetEntriesEncodedStreamingDF.redisEvent).cast("string"))
...
stediEventsStreamingDF.withColumn("value", from_json("value", customerScoreSchema)) \
    .select(col('value.*')) \
...
customerRiskAndBirthYearDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr( """
   customer=email
"""))
...
customerRiskAndBirthYearDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value") \
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
    .option("topic", "graph-data") \
